...
// Ported from https://gist.github.com/OmarAflak/aca9d0dc8d583ff5a5dc16ca5cdda86a
            ...
            for (j=0 ; j<newImageWidth ; j++) {
                for (h=i ; h<i+filterHeight ; h++) {
                    for (w=j ; w<j+filterWidth ; w++) {
                        switch (d)
                        {
                        case 0: newImage->data[i][j].red   += (int) (filter->buf[h-i][w-j] * (double) image->data[h][w].red); break;
                        case 1: newImage->data[i][j].green += (int) (filter->buf[h-i][w-j] * (double) image->data[h][w].green); break;
                        case 2: newImage->data[i][j].blue  += (int) (filter->buf[h-i][w-j] * (double) image->data[h][w].blue); break;
                        }
                    }
                }
            }
